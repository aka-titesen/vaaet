...
Un posible accidente requiere revisión humana. La confirmación se registra con `human_override_state=3` e `is_human_validated=true`; una alerta automática nunca constituye ground truth. El holdout marcado como `test` queda congelado y no puede utilizarse para ajustar reglas, umbrales, calibración o arquitectura.
