...
def _validate_package_origin(package: ModuleType, ml_root: Path, in_colab: bool) -> Path:
    package_file = getattr(package, "__file__", None)
    if not package_file:
        resolved_paths = list(getattr(package, "__path__", ()))
        raise RuntimeConfigurationError(
            "The 'vaaet' import resolved to a namespace package; rerun the environment cell. "
            f"Resolved locations: {resolved_paths}"
        )
    origin = Path(package_file).resolve()
    expected_editable_root = (ml_root / "src" / "vaaet").resolve()
    if in_colab and ml_root.resolve() in origin.parents:
        raise RuntimeConfigurationError(
            f"Colab must load the installed package, not the checkout: {origin}"
        )
...
def bootstrap_notebook_runtime(
    *,
    ...
    if not (workspace_root / ".git").is_dir() or not (ml_root / "pyproject.toml").is_file():
        raise RuntimeConfigurationError("VAAET workspace or vaaet-ml component was not found.")

    _clear_vaaet_modules()
    import vaaet

    package_file = _validate_package_origin(vaaet, ml_root, in_colab)
